...
// --- UPDATED TRANSLATOR FOR NOCCCD'S WEIRD DAY OBJECTS ---
function parseNocccdDays(meeting: any): string[] {
  const daysArray: string[] = [];
  
  // Cypress creates explicit keys for each day if it meets on that day
...
// NOCCCD times come as "1800" instead of "18:00". Let's format them nicely for the frontend.
    ...
    for (const course of courses) {
      ...
      await prisma.course.upsert({
        ...
        create: {
          ...
          // --- THE REAL FIX: Parsing the correct keys! ---
          meetings: {
            create: (course.sectMeetings || []).map((m: any) => ({
              // Pass the whole meeting object to check for M, T, W, R keys
              days: parseNocccdDays(m), 
              
              // Use beginTime and endTime, and format them nicely!
              startTime: formatTime(m.beginTime),
              endTime: formatTime(m.endTime)
              
            })).filter((m: any) => m.startTime !== "") // Ensure we don't save empty times
          }
        }
      });
    }
