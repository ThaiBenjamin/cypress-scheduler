...
# The Master List of Semesters - Now with BOTH files!
TERMS = [
    {
        "name": "2026-Summer", 
        ...
        for term in TERMS:
            ...
            if catalog_response.status_code == 200:
                ...
                for course in catalog_data:
                    ...
                    subj = str(course.get("crseSubjCode", "")).replace(" ", "")
                    numb = str(course.get("crseCrseNumb", "")).replace(" ", "")
                    unique_id = subj + numb
            ...
            if sections_response.status_code == 200:
                ...
                for section in sections_data:
                    # FILTER: Only keep Cypress College (" C") classes!
                    course_num_raw = str(section.get("sectCrseNumb", ""))
                    if course_num_raw.endswith(" C"):
                        
                        # Find the matching title and units from our dictionary
                        subj = str(section.get("sectSubjCode", "")).replace(" ", "")
                        numb = course_num_raw.replace(" ", "")
                        lookup_id = subj + numb
